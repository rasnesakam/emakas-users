...
public class StringToResourcePermissionConverter implements Converter<String, Optional<ResourcePermission>> {
    ...
    public Optional<ResourcePermission> convert(String source) {
        ...
        if (split.length == 3) {
            PermissionScope permissionScope = PermissionScope.valueOf(split[INDEX_OF_PERMISSION_SCOPE]);
            AccessModifier accessModifier = AccessModifier.valueOf(split[INDEX_OF_ACCESS_MODIFIER]);
